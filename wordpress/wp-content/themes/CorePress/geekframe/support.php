    ...
    if (get_user_option('rich_editing') == 'true' || get_user_option('rich_editing') == 'false') {
        ...
        <script>
            jQuery(document).ready(function () {
                layer.config({
                    extend: 'corepress/style.css?v=4.8',
                    skin: 'corepress-layer'
                });
                jQuery('.corepress-btn').click(() => {
                    jQuery('.corepress-btn').text('加载中，请稍后');
                    jQuery.get('<?php echo admin_url('admin-ajax.php');?>', {action: 'geteditwindowhtml'}, function (str) {
                        jQuery('.corepress-btn').text('CorePress编辑器增强');
                        layer.open({
                            type: 1,

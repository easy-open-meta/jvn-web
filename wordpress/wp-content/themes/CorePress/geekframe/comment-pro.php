...
function my_comment($comment, $args, $depth)
{
    ...
    if ($comment->comment_author_url != '') {
        $zhanzhang = '<span class="comment-zhan"><img title="网站主" src="' . file_get_img_url('zhan.svg') . '" alt=""></span>';
    } else {
        $zhanzhang = '';
    }
    ...
    <div class="comment-item" id="comment-<?php comment_ID(); ?>">
        <div class="comment-media">
            <div class="avatar-img">
                <img src="<?php echo corepress_get_avatar_url($comment->comment_author_email, 48) ?>" alt="">
            </div>
        </div>
        <div class="comment-metadata">
            <div class="media-body">
                <?php echo __('<p class="author_name">') . get_comment_author_link() . $user . $reply . $zhanzhang . '</p>'; ?>
            ...
            <span class="comment-btn-reply">
 				 <?php comment_reply_link(array_merge($args, array('reply_text' => '<i class="fa fa-reply"></i> 回复', 'depth' => $depth, 'max_depth' => $args['max_depth']))) ?>
   			</span>
        </div>
    </div>
